...
import { SearchkitComponent, ViewSwitcherHits, NoHits } from "searchkit";
    ...
    const getData = async () => {
      ...
      await hits.map((hit) => {
        let row = {
          ...
          deliveryFrom: moment(hit._source.DELIVERY_FROM_DAT).format(
            "YYYY-MM-DD  hh:mm:ss"
          ),
          deliveryTo: moment(hit._source.DELIVERY_TO_DAT).format(
            "YYYY-MM-DD  hh:mm:ss"
          ),
    ...
    return (
      <div className="table-container">
        <ViewSwitcherHits
          ...
          hitComponents={[
            {
              key: config.samples.table.key,
              title: config.samples.table.title,
              listComponent: OrderHitsTable,
            },
          ]}
          scrollTo="body"
        />
